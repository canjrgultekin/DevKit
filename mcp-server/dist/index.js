...
"API ayakta mi", "health check", "endpoint test et", "servisi kontrol et" dediginde CAGIR.`,
    ...
    const data = await devkitApi("shell/exec", "POST", { command: curlCmd, shell: "cmd", timeoutSeconds: (timeoutSeconds || 5000) + 5 });
...
function readCredential(topic, ...keys) {
    const data = jarwisReadJson(`credentials/${topic}.json`);
    if (!data)
        return null;
    for (const k of keys) {
...
server.registerTool("jarwis_scan_pc", {
    ...
    for (const [tool, cmd] of Object.entries(checks)) {
        try {
            const result = await devkitApi("shell/exec", "POST", { command: cmd, shell: "cmd", timeoutSeconds: 5000 });
...
// ═══════════════════════════════════════════════
server.registerTool("devkit_remote_host_add", {
...
server.registerTool("devkit_remote_host_list", {
...
server.registerTool("devkit_remote_host_remove", {
...
server.registerTool("devkit_remote_host_test", {
...
server.registerTool("devkit_remote_exec", {
...
// ═══════════════════════════════════════════════
server.registerTool("devkit_llm_provider_add", {
...
server.registerTool("devkit_llm_provider_list", {
...
server.registerTool("devkit_llm_provider_remove", {
...
server.registerTool("devkit_llm_provider_test", {
...
server.registerTool("devkit_llm_ask", {
...
// ═══════════════════════════════════════════════
server.registerTool("jarwis_save_session", {
...
Her context turu opsiyoneldir; o session'da hangi tur is yapildiysa onu doldur: programming (programlama/gelistirme isleri), browser (tarayici islemleri ve NASIL yapildigi, ki ileride sifirdan kesif yapilmasin), localpc (local pc islemleri). summary genel session ozeti, topics konu etiketleridir. jarwis-context genel hafizasi ve aktif session log'u her durumda guncellenir.`,
    ...
    const mergeWrite = (fp, obj) => {
        const existing = jarwisReadJson(fp) || {};
    ...
    if (mc?.stats?.lastSessionId) {
        ...
        const log = jarwisReadJson(`session-logs/${sid}.json`);
        if (log) {
            log.summary = summary;
            if (topics)
                log.topics = topics;
...
server.registerTool("jarwis_save_credentials", {
...
server.registerTool("jarwis_load_credentials", {
...
topic verilirse o dosyayi doner, bos birakilirsa kayitli tum credential dosyalarini listeler.`,
    ...
    if (!topic) {
        if (!existsSync(credDir))
            return { content: [{ type: "text", text: "Henuz credential kaydi yok." }] };
